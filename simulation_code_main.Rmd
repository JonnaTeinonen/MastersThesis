...
# install_version("mice",version = "3.15.0")
# detach("package:nnet", unload = TRUE)
# install_version("nnet",version = "7.3-19")
# install_version("ResourceSelection", version = "0.3-5")
...
for (ss in sample_sizes){
  for (rr in response_rates){
    ...
    sim_files <- list.files(path = paste0("C:\\...\\Project\\simulation\\robust\\n", ss, "\\RR", rr),
...
for (ss in sample_sizes){
  for (rr in response_rates){
   # List for each MICE variant for the estimated parameter results
    ...
    # Path where the imputed data for scenario 1 is located
    sim_files <- list.files(path = paste0("C:\\...\\Project\\simulation\\mice\\n", ss, "\\RR", rr),
                                          pattern = ".rds", full.names = T)
...
for (ss in sample_sizes){
  for (rr in response_rates){
    ...
    sim_files <- list.files(path = paste0("C:\\...\\Project\\simulation\\CCA\\n", ss, "\\RR", rr),
                                          pattern = ".rds", full.names = T)
